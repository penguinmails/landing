...
- Signup/CTA links go through `buildSignupUrl` in `src/lib/cta.ts`, which takes a `context`, an optional `plan`, and an optional `billing` period and returns a fully-formed URL with `utm_source`, `utm_medium`, `utm_campaign`, `plan`, and `billing` params attached.
...
The boundary is enforced by convention rather than code. Plain URL constants (`PUBLIC_APP_ROOT_URL`, `PUBLIC_WATCH_DEMO_URL`, `PUBLIC_BOOK_DEMO_EMAIL`) are imported directly from `astro:env/client` in the components that need them. `src/lib/cta.ts` is scoped to `buildSignupUrl` only — it owns the signup URL construction logic, not the other constants. All values are defined in the `env.schema` in `astro.config.mjs`.
